...
public class ChoiceBase<T0, T1, T2, T3, T4> : IChoice
{
    ...
    protected ChoiceBase(Choice<T0?, T1?, T2?, T3?, T4?> input)
    {
        Index = input.Index;
        switch (Index)
        {
    ...
    private bool Equals(ChoiceBase<T0, T1, T2, T3, T4> other) =>
        Index == other.Index &&
        Index switch
        {
    ...
    public override bool Equals(object? obj)
    {
        ...
        return obj is ChoiceBase<T0, T1, T2, T3, T4> o && Equals(o);
    }
